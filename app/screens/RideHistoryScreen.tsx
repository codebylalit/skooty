...
export default function RideHistoryScreen() {
  ...
  return (
    <View style={tw`flex-1 bg-white px-4 pt-8`}>
      <Text style={tw`text-2xl font-bold text-blue-600 mb-6 text-center`}>Ride History</Text>
      <FlatList
        data={rides}
        ...
        renderItem={({ item }) => (
          <View style={tw`mb-4 border border-gray-200 rounded-xl p-4 bg-gray-50`}> 
            ...
            <Text style={tw`text-base text-gray-500 mb-1`}>Date: {item.createdAt?.toDate ? item.createdAt.toDate().toLocaleString() : 'N/A'}</Text>
            {item.pickup && (
              <Text style={tw`text-base text-gray-400`}>Pickup: Lat {item.pickup.latitude?.toFixed(5)}, Lon {item.pickup.longitude?.toFixed(5)}</Text>
            )}
          </View>
        )}
